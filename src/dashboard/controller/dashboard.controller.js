  ...
  try {
    ...
    const requests = pendingRequests.map(req => {
      ...
      return {
        ...
        image: userImage ? (userImage.startsWith('http') ? userImage : `${process.env.BACKEND_URL}${userImage}`) : "https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=100"
      };
    });
...
export const userDashboard = async (req, res) => {
  try {
    ...
    if (nextBookingDoc) {
      ...
      const image = rawImage ? (rawImage.startsWith('http') ? rawImage : `${process.env.BACKEND_URL}${rawImage}`) : "https://images.unsplash.com/photo-1577219491135-ce39a73e4f83?auto=format&fit=crop&q=80&w=300";
